...
from deep_cbc.utils.constants import TENSORBOARD_LOG_DIR
...
def train_cbcnet(
...
):  # Set tensor_log_flag as False to remove gradient logging.
    ...
    if pretrain:
        ...
        t_weight = 5.0
        cl_weight = 0.0
    else:  # All other loss functions are ignored in classification training step implementation.
        if net.module._args.head_type in ("rbf_head", "pipnet_head"):
            align_pf_weight = 5.0
            t_weight = 2.0
            unif_weight = 0.0
            cl_weight = 2.0
    ...
    for i, (xs1, xs2, ys) in train_iter:
        ...
        # Different loss function signatures for different model heads involved for training.
        if net.module._args.head_type in ("rbf_head", "pipnet_head"):
            loss, acc = calculate_loss(
                ...
                criterion,
                train_iter,
                net.module._args.head_type,
                net.module._num_classes,
                EPS=1e-8,
            )
        else:  # For "cbc_head" reasoning_labels are also parsed as arguments.
            loss, acc = calculate_loss(
                proto_features,
                pooled,
                out,
                ...
                net.module._args.head_type,
                net.module._num_classes,
                reasoning_labels=net.module._classification.reasoning_labels,
                EPS=1e-8,
            )
        ...
        # Logging the gradient training logs for the CBCNet model training.
        if i % 10 == 0 and tensor_log_flag:
            if not (Path(net.module._args.args.log_dir) / TENSORBOARD_LOG_DIR).is_dir():
                (Path(net.module._args.args.log_dir) / TENSORBOARD_LOG_DIR).mkdir(
                    parents=True, exist_ok=True
                )
            tensorboard_logger = SummaryWriter(
                Path(net.module._args.args.log_dir) / TENSORBOARD_LOG_DIR
            )
            log_gradients_in_model(net, tensorboard_logger, i)
        ...
        if not pretrain:
            with torch.no_grad():
                # Weight clamping enforced for PIPNet and RBF architectures.
                if net.module._args.head_type in ("pipnet_head", "rbf_head"):
                    ...
                    net.module._classification.weight.copy_(
                        torch.clamp(
                            net.module._classification.weight.data - 1e-3, min=0.0
                        )
                    )
...
def log_gradients_in_model(model, tensorboard_logger, step):
    for tag, value in model.named_parameters():
        if value.grad is not None:
            tensorboard_logger.add_histogram(tag + "/grad", value.grad.cpu(), step)
...
def calculate_loss(
    ...
    if not pretrain:
        # Handling heads of PIPNet and RBFNet models.
        if head_type in ("pipnet_head", "rbf_head"):
            softmax_inputs = torch.log1p(out**net_normalization_multiplier)
            class_loss = criterion(F.log_softmax((softmax_inputs), dim=1), ys)
        else:  # Handling heads of CBCNet models.
            # detection probabilities, reasoning_labels and labels needed for the MarginLoss function.
            class_loss = criterion(
                out, reasoning_labels, F.one_hot(ys, num_classes=num_classes).float()
            )
